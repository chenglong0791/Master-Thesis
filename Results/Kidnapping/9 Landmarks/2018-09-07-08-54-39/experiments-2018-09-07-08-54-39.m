...
pauseTime             = 0.01;          % Time between two consecutive updates of live plots in seconds
...
timePF           = 0;                          % Time duration of estimation PF
timeUPF          = 0;                          % Time duration of estimation UPF
timePFC          = 0;                          % Time duration of estimation PFC
timeUPFC         = 0;                          % Time duration of estimation UPFC
timePFS          = 0;                          % Time duration of estimation PFS
timeUPFS         = 0;                          % Time duration of estimation UPFS
...
for runIndex = 1:nRuns
    ...
    if PF
        ...
        elapsedTimePF = toc;
        timePF = timePF + elapsedTimePF;
        [rmsErrorsPF(runIndex, 1), errorsPF(runIndex, :)] = computeErrors(position, xhPF(:, :, runIndex));
        
        diary on
        fprintf(['Elapsed time:            %3.1f sec', newline ], elapsedTimePF);
    ...
    if UPF
        ...
        % Compute elapse time and errors
        elapsedTimeUPF = toc;
        timeUPF = timeUPF + elapsedTimeUPF;
        [rmsErrorsUPF(runIndex, 1), errorsUPF(runIndex, :)] = computeErrors(position, xhUPF(:, :, runIndex));
        
        diary on
        fprintf(['Elapsed time:            %3.1f sec', newline ], elapsedTimeUPF);
    ...
    if PFC
        ...
        % Compute elapse time and errors
        elapsedTimePFC = toc;
        timePFC = timePFC + elapsedTimePFC;
        [rmsErrorsPFC(runIndex, 1), errorsPFC(runIndex, :)] = computeErrors(position, xhPFC(:, :, runIndex));
        
        diary on
        fprintf(['Elapsed time:            %3.1f sec', newline], elapsedTimePFC);
    ...
    if UPFC
        ...
        % Compute elapse time and errors
        elapsedTimeUPFC = toc;
        timeUPFC = timeUPFC + elapsedTimeUPFC;
        [rmsErrorsUPFC(runIndex, 1), errorsUPFC(runIndex, :)] = computeErrors(position, xhUPFC(:, :, runIndex));
        
        diary on
        fprintf(['Elapsed time:            %3.1f sec', newline], elapsedTimeUPFC);
    ...
    if PFS
        ...
        % Compute elapse time and errors
        elapsedTimePFS = toc;
        timePFS = timePFS + elapsedTimePFS;
        [rmsErrorsPFS(runIndex, 1), errorsPFS(runIndex, :)] = computeErrors(position, xhPFS(:, :, runIndex));
        
        diary on
        fprintf(['Elapsed time:            %3.1f sec', newline], elapsedTimePFS);
    ...
    if UPFS
        ...
        % Compute elapse time and errors
        elapsedTimeUPFS = toc;
        timeUPFS = timeUPFS + elapsedTimeUPFS;
        [rmsErrorsUPFS(runIndex, 1), errorsUPFS(runIndex, :)] = computeErrors(position, xhUPFS(:, :, runIndex));
        
        diary on
        fprintf(['Elapsed time:            %3.1f sec', newline], elapsedTimeUPFS);
...
meanRmsErrorsPF       = mean(rmsErrorsPF, 1);
meanRmsErrorsUPF      = mean(rmsErrorsUPF, 1);
meanRmsErrorsPFC      = mean(rmsErrorsPFC, 1);
meanRmsErrorsUPFC     = mean(rmsErrorsUPFC, 1);
meanRmsErrorsPFS      = mean(rmsErrorsPFS, 1);
meanRmsErrorsUPFS     = mean(rmsErrorsUPFS, 1);
...
relPerformanceUPF     = 100 * meanRmsErrorsUPF  / meanRmsErrorsPF;
relPerformancePFC     = 100 * meanRmsErrorsPFC  / meanRmsErrorsPF;
relPerformanceUPFC    = 100 * meanRmsErrorsUPFC / meanRmsErrorsPF;
relPerformancePFS     = 100 * meanRmsErrorsPFS  / meanRmsErrorsPF;
relPerformanceUPFS    = 100 * meanRmsErrorsUPFS / meanRmsErrorsPF;
...
meanTimePF            = timePF   / nRuns;
meanTimeUPF           = timeUPF  / nRuns;
meanTimePFC           = timePFC  / nRuns;
meanTimeUPFC          = timeUPFC / nRuns;
meanTimePFS           = timePFS  / nRuns;
meanTimeUPFS          = timeUPFS / nRuns;

relTimeUPF            = timeUPF  / timePF;
relTimePFC            = timePFC  / timePF;
relTimeUPFC           = timeUPFC / timePF;
relTimePFS            = timePFS  / timePF;
relTimeUPFS           = timeUPFS / timePF;

fprintf(['--- Overall performance ---', newline2]);

fprintf(['Mean RMSE:                  ',   newline ]);
fprintf(['PF:                        %2.2f m',   newline ],  meanRmsErrorsPF);
fprintf(['UPF:                       %2.2f m',   newline ],  meanRmsErrorsUPF);
fprintf(['PFC:                       %2.2f m',   newline ],  meanRmsErrorsPFC);
fprintf(['UPFC:                      %2.2f m',   newline ],  meanRmsErrorsUPFC);
fprintf(['PFS:                       %2.2f m',   newline ],  meanRmsErrorsPFS);
fprintf(['UPFS:                      %2.2f m',   newline2],  meanRmsErrorsUPFS);
...
if plotFirstNSeconds
    ...
    plotErrors(t(1:index), [meanErrorsPF(1:index); meanErrorsUPF(1:index); ...
        ...
        [meanRmsErrorsPF; meanRmsErrorsUPF; meanRmsErrorsPFC; meanRmsErrorsUPFC;
        meanRmsErrorsPFS; meanRmsErrorsUPFS], ...
...
plotErrors(t, [meanErrorsPF; meanErrorsUPF; meanErrorsPFC; meanErrorsUPFC; meanErrorsPFS; meanErrorsUPFS], ...
    [meanRmsErrorsPF; meanRmsErrorsUPF; meanRmsErrorsPFC; meanRmsErrorsUPFC; meanRmsErrorsPFS; meanRmsErrorsUPFS], ...
...
plotTimes([timeUPF, timePFC, timeUPFC, timePFS, timeUPFS] / timePF, ...
    [fileName, ' -- mean computation time relative to PF after ', num2str(nRuns), ' runs'], ...
    {'UPF', 'PFC', 'UPFC', 'PFS', 'UPFS'});

plotInitErrors([meanErrorsPF(1), meanErrorsUPF(1), meanErrorsPFC(1), ...
    meanErrorsUPFC(1), meanErrorsPFS(1), meanErrorsUPFS(1)], ...
    [fileName, ' -- mean initial errors after ', num2str(nRuns), ' runs']);
