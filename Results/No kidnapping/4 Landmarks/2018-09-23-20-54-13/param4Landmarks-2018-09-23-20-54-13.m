...
load('Trajectory_4_with_4_landmarks.mat');  % Load data

nParticlesPF          = 1000;       % Number of particles generic   particle filter
nParticlesUPF         = 10;       % Number of particles unscented particle filter
nParticlesPFC         = 1000;       % Number of particles generic   particle filter with contractor
nParticlesUPFC        = 10;       % Number of particles unscented particle filter with contractor
nParticlesPFS         = 1000;       % Number of particles generic   particle filter with SIVIA
...
sigmaSysNoisePFC      = 0.5 * 100  * [sigmaVelocity * ones(1,3), sigmaEulerAngles * ones(1,3)];
...
sigmaLikelihoodUPFC   = 1 * 10   * sigmaDistances;
sigmaLikelihoodUPFS   = 0.5 * 10 * sigmaDistances;

sigmaTransPriorUPF    = 1 * 1000 * (sigmaVelocity + sigmaEulerAngles);
sigmaTransPriorUPFC   = 0.5 * 1000 * (sigmaVelocity + sigmaEulerAngles);
sigmaTransPriorUPFS   = 0.1 * 1000 * (sigmaVelocity + sigmaEulerAngles);
