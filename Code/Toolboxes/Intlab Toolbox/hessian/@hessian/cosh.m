function r = cosh(a)
%COSH         Hessian (elementwise) hyperbolic cosine
%
...
%
  ...
  if K==1                   % scalar hessian
    ...
    sinhax = sinh(a.x);
    r.dx = sinhax * a.dx;
    r.hx = sinhax * a.hx + reshape( ((0.5*r.x)*a.dx) * a.dx.' , size(a.hx) );
    ...
    if issparse(a.hx)               % input sparse
      ...
      rx = r.x(:).';
      sinhax = sinh(a.x(:).');
      sinhax = sinhax(ones(N*N,1),:);
      r.dx = a.dx .* sinhax(1:N,:);
      adx = repmat(0.5*rx,N,1) .* a.dx;
      r.hx = a.hx .* sinhax + adx(repmat(1:N,N,1),:) .* a.dx(repmat(1:N,1,N),:);
