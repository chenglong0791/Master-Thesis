...
%
  ...
  if K==1                   % scalar hessian
    ...
    cosax = cos(a.x);
    r.dx = cosax * a.dx;
    r.hx = cosax * a.hx - reshape( ((0.5*r.x)*a.dx) * a.dx.' , size(a.hx) );
    ...
    if issparse(a.hx)               % input sparse
      ...
      rx = r.x(:).';
      cosax = cos(a.x(:).');
      cosax = cosax(ones(N*N,1),:);
      r.dx = a.dx .* cosax(1:N,:);
      adx = repmat(0.5*rx,N,1) .* a.dx;
      r.hx = a.hx .* cosax - adx(repmat(1:N,N,1),:) .* a.dx(repmat(1:N,1,N),:);
