function r = sinh(a)
%SINH         Hessian (elementwise) hyperbolic sine
%
...
%
  ...
  if K==1                   % scalar hessian
    ...
    coshax = cosh(a.x);
    r.dx = coshax * a.dx;
    r.hx = coshax * a.hx + reshape( ((0.5*r.x)*a.dx) * a.dx.' , size(a.hx) );
    ...
    if issparse(a.hx)               % input sparse
      ...
      rx = r.x(:).';
      coshax = cosh(a.x(:).');
      coshax = coshax(ones(N*N,1),:);
      r.dx = a.dx .* coshax(1:N,:);
      adx = repmat(0.5*rx,N,1) .* a.dx;
      r.hx = a.hx .* coshax + adx(repmat(1:N,N,1),:) .* a.dx(repmat(1:N,1,N),:);
