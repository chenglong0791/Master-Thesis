function u = tanh(a)
%TANH         Slope hyperbolic tangent tanh(a)
%
...
%
  ...
  if any(index)
    aindex.r = a.r(index,:);
    aindex.s = a.s(index,:);
    u.s(index,:) = slopeconvexconcave('tanh','1./sqr(cosh(%))',aindex,1);
    Index(index) = 0;
  end
  ...
  if any(index)
    aindex.r = a.r(index,:);
    aindex.s = a.s(index,:);
    u.s(index,:) = slopeconvexconcave('tanh','1./sqr(cosh(%))',aindex,0);
    Index(index) = 0;
  end

  if any(Index)
    Index( Index==0 ) = [];
    u.s(Index,:) = a.s(Index,:) ./ sqr(cosh(Xxs(Index)));
  end
