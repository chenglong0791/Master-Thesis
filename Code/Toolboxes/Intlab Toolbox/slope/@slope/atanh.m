function u = atanh(a)
%ATANH        Slope inverse hyperbolic tangent atanh(a)
%
...
%
  ...
  if any(index)
    aindex.r = a.r(index,:);
    aindex.s = a.s(index,:);
    u.s(index,:) = slopeconvexconcave('atanh','1./(1-sqr(%))',aindex,0);
    Index(index) = 0;
  end
  ...
  if any(index)
    aindex.r = a.r(index,:);
    aindex.s = a.s(index,:);
    u.s(index,:) = slopeconvexconcave('atanh','1./(1-sqr(%))',aindex,1);
    Index(index) = 0;
  end

  if any(Index)
    Index( Index==0 ) = [];
    u.s(Index) = a.s(Index) ./ ( 1 - sqr(Xxs(Index)) );
  end
