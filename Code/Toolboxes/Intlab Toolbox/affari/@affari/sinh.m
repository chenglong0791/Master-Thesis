function r = sinh(a,see)
%SINH         Affine arithmetic elementwise hyperbolic sine  sinh(a)
%
...
%
  ...
  if INTLAB_CONST.AFFARI_APPROX         % min-range approximation
    ...
    elseif any(indexpos(:))
      ...
      r = sinhpos(r,indexpos,neg,aa,see);
    end
    ...
    if see && ( numel(a.mid)==1 )
      if neg
        showgraph('sinh(x)',p,-q,delta,-a.range)
      else
        showgraph('sinh(x)',p,q,delta,a.range)
      end
    end
...
function r = sinhpos(r,indexpos,neg,a,see)
...
end  % function sinhpos
    

function r = sinh0(r,index0,a,see)
