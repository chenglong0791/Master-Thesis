function r = asinh(a,see)
%ASINH        Affine arithmetic elementwise inverse hyperbolic sine  sinh(a)
%
...
%
  ...
  if INTLAB_CONST.AFFARI_APPROX     % min-range approximation
    ...
    elseif any(indexpos(:))
      ...
      r = asinhpos(r,indexpos,neg,aa,see);
    end
    ...
    if any(narrow(:))                        % slope in f'(X)
      p(narrow) = 1./sqrt( 1 + sqr(X(narrow)) );
      xi(narrow) = X(narrow);
    ...
    if see && ( numel(a.mid)==1 )
      if neg
        showgraph('asinh(x)',p,-q,delta,-a.range)
      else
        showgraph('asinh(x)',p,q,delta,a.range)
      end
    end
  ...
  % improve range
  setround(1)
  r = intersectNaN( r , asinh(a.range) );
  setround(rndold)
...
function r = asinhpos(r,indexpos,neg,a,see)
% non-negative arguments for min-range approximation
  ...
  p = 1./sqrt( 1+sqr(X2) );                   % inclusion of slope
...
end  % function asinhpos
    

function r = asinh0(r,index0,a,see)
% arguments enclosing zero for min-range approximation
  ...
  if X.sup>-X.inf
    p = 1./sqrt(sqr(intval(X.sup))+1);
  else
    p = 1./sqrt(sqr(intval(X.inf))+1);
  end
  fXinf = intval(fX.inf);
  Xsup = intval(X.sup);
