function r = acot(a,see)
%ACOT         Affine arithmetic elementwise inverse tangent  acot(a)
%
...
%
  ...
  elseif any(indexpos(:))
    ...
    r = acotpos(r,indexpos,neg,aa,see);
  end
    
  % improve range
  setround(1)
  r = intersectNaN( r , acot(a.range) );
  setround(rndold)
...
function r = acotpos(r,indexpos,neg,a,see)
% non-negative arguments
  ...
  if INTLAB_CONST.AFFARI_APPROX
    ...
    if any(index(:))                          % slope in f'(X)
      p(index) = -1 ./ ( 1 + sqr(a.range(index)) );
      xi(index) = a.range(index);
    end
    delta = 0.5*( acot(xi) - fx1 - p.*(xi-x1) );
  ...
  if see && ( numel(a.mid)==1 )
    if neg
      showgraph('acot(x)',p,-q,delta,-a.range)
    else
      showgraph('acot(x)',p,q,delta,a.range)
    end
  end
