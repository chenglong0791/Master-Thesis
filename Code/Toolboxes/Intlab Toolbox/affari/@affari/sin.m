function r = sin(a,see)
%SIN          Affine arithmetic elementwise sine  sin(a)
%
...
%
  ...
  Pi = intval('pi');
  fX = sin(a.range);                % before range reduction
  [a,huge,Sinf,Ssup] = rangereduction(a,3);
  ...
  if INTLAB_CONST.AFFARI_APPROX     % min-range approximation
    ...
    if any(~huge)
      ...
      p = intersect(p,cos(X));              % small rad(X)
      xi1 = acos(p);                        % in [0,pi]
      ...
      if any(index)
        xi2(index) = xi1(index);            % in quartil 1..2
        xi1(index) = -xi1(index);           % in quartil -1..0
      end
      index = ( Sinf==1 );                  % 1<=Ssup<=3
      if any(index)
        xi2(index) = 2*Pi - xi1(index);     % in quartil 3..4
      end
      % 0 in p treated as 'narrow'
      index = ( Sinf==2 ) & ( p(:)>0 );     % 2<=Ssup<=5
      if any(index)
        xi2(index) = 2*Pi - xi1(index);     % in quartil 4
        xi1(index) = xi1(index) + 2*Pi;     % in quartil 5
      end
      index = ( Sinf==2 ) & ( p(:)<0 );     % 2<=Ssup<=5
      if any(index)
        xi2(index) = 2*Pi - xi1(index);     % in quartil 3
      end
      index = ( Sinf==3 );                  % 3<=Ssup<=5
      if any(index)
        xi2(index) = 2*Pi - xi1(index);     % in quartil 3..4
        xi1(index) = 2*Pi + xi1(index);     % in quartil 5..6
      end
      narrow = in(0,p(:)) | isnan(p(:)) | isinf(p(:)) | ...
        in(0,xi1(:)) | isnan(xi1(:)) | isinf(xi1(:));
      if any(narrow(:))                     % slope in f'(X)
        p(narrow) = cos(X(narrow));
    ...
    if see && ( numel(a.mid)==1 )
      showgraph('sin(x)',p,q,delta,a.range)
    end
...
function r = sin_(r,index,left,a,see)
...
function r = sin0(r,index0,a,see)
