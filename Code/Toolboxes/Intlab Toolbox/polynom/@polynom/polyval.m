...
% modified 01/30/16     S.M. Rump  code improvement (thanks to Florian Bünger)
...
%
  ...
  if size(p.e,2)==1                         % univariate case
    if isa(p.c,'intval') || isa(x,'intval')
      INTLAB_POLYNOM_UEVAL = INTLAB_CONST.POLYNOM_UEVAL;
    ...
    if iscell(x)                            % x is cell array (for subpolynomials by empty components)
      ...
      INTLAB_POLYNOM_MEVAL = INTLAB_CONST.POLYNOM_MEVAL;
