...
%
  ...
  if nargin==0
    ...
    INTLAB_MATRIX_MEMORY_ALL = sparse([],[],[]);    % matrix memory for complete path (to be built)
    INTLAB_MATRIX_FILES_ALL = [];
    INTLAB_MATRIX_MEMORY_USER = sparse([],[],[]);   % matrix memory for user-defined path (to be built)
    INTLAB_MATRIX_FILES_USER = [];
    INTLAB_MATRIX_MEMORY = sparse([],[],[]);        % matrix memory for user-defined path (to be built)
    INTLAB_MATRIX_FILES = [];
    
    INTLAB_CONST.MATRIX_MEMORY_ALL = INTLAB_MATRIX_MEMORY_ALL; 
    INTLAB_CONST.MATRIX_FILES_ALL = INTLAB_MATRIX_FILES_ALL;
    INTLAB_CONST.MATRIX_MEMORY_USER = INTLAB_MATRIX_MEMORY_USER; 
    INTLAB_CONST.MATRIX_FILES_USER = INTLAB_MATRIX_FILES_USER;
    INTLAB_CONST.MATRIX_MEMORY = INTLAB_MATRIX_MEMORY;
    INTLAB_CONST.MATRIX_FILES = INTLAB_MATRIX_FILES;
    
    dirold = cd;
    P = [path pathsep];
    
    p = which('spones');            % compute Matlab path
    i = strfind(p,['toolbox' filesep]);
    MatlabPath = p(1:i-1);
    
    p = which('intvalinit');        % compute INTLAB path
    i = strfind(p,['intval' filesep]);
    INTLABPath = p(1:i-1);
    ...
    while ~isempty(P)
      ...
      if isempty(strfind(p,MatlabPath))
  ...
  if nargin==1                                  % input only file name, no extra parameter
    MEMORY = INTLAB_CONST.MATRIX_MEMORY;
    FILES = INTLAB_CONST.MATRIX_FILES;
    level = defaultlevel - max(0,5-length(name));  % level smaller for short names
  else                                          % at least two parameters
    all = lower(all);
    if isequal(all,'all')
      MEMORY = INTLAB_CONST.MATRIX_MEMORY_ALL; 
      FILES = INTLAB_CONST.MATRIX_FILES_ALL;
      if nargin~=3
        level = defaultlevel - max(0,5-length(name));  % level smaller for short names
      end
    elseif isequal(all,'user')
      MEMORY = INTLAB_CONST.MATRIX_MEMORY_USER; 
      FILES = INTLAB_CONST.MATRIX_FILES_USER;
      if nargin~=3
  ...
  if isempty(strfind(name,'.'))                 % add file extension .m if not specified
    name = [name '.m'];
  end
...
end  % function appand_matrix_memory
