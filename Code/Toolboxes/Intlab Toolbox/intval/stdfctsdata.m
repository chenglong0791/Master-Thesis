...
% modified 04/19/14     S.M. Rump  save with -mat option
...
%
  ...
  INTLAB_CONST.STDFCTS_EPSMAX = 1e-15;
  ...
  if ~success
    NoSuccess(ALLDATA,errorterm,prec);
  end
  disp(' ')
...
end  % function stdfctsdata
  
  

function NoSuccess(ALLDATA,errorterm,prec)
...
end  % function CheckSpecialValues
...
end  % function CheckSpecialValues_
...
end  % function EstimateExpSinCosTanSinhTanh
...
end  % function EstimateExpSinCosTanSinhTanh_
...
% check floating point error against long, check max error
  ...
  INTLAB_STDFCTS_EPSMAX = INTLAB_CONST.STDFCTS_EPSMAX;
  success = 1;
