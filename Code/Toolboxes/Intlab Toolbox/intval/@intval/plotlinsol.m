...
%Because of that the picture is not completely rigorous! In particular for
%border line situations (close to singularity) rounding errors may
%jeopardize the result.
%If the interval matrix contains a singular matrix, an error message is given.
%
...
% modified 10/31/14     S.M. Rump  detection of singular matrix corrected (Thanks to Sergey Shary)
...
function Sigma = plotlinsol3(A,b,printaxes,printvertexsol) 
  ...
  for j=0:(2^n-1)
    ...
    S = diag(2*v-1);
    % check for singularity using Rohn's theorem
    for j1=0:(2^n-1)
      v1 = dec2bin(j1,n) - 48;           % double('0') = 48
      S1 = diag(2*v1-1);
      if ( D*det(mA-S1*rA*S)<=0 )
        Sigma = NaN;
        error('Interval matrix contains a singular matrix')
        return
      end
    end
    M = [ S ; rA*S-mA ; mA+rA*S ];
