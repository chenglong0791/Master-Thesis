...
% modified 07/15/13     S.M. Rump  zero midpoint matrix
% modified 07/16/13     S.M. Rump  radius corrected
% modified 07/17/13     S.M. Rump  normposA into private folder
...
% modified 08/09/17     S.M. Rump  typo, thanks to Florian Bünger
%
  if nargin==1
    r = 2;
  end
  ...
  if ( m==1 ) || ( n==1 )        % vector a
    ...
    elseif isequal(r,2) || isequal(r,'fro')
    ...
    elseif r==2                 % matrix spectral norm
      ...
      % take care of midpoint zero matrix
      if ~any(any(a))
        res = infsup(0,mag(normR));
      ...
      if ~symm                  % prepare for general case
        ...
        if a.complex            % complex matrix
          ...
          a = amid-a.inf;
          setround(0)
          normR = normR + sqrt(normposA(a,acc));
