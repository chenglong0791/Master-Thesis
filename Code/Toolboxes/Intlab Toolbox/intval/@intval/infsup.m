...
% modified 09/05/15     S.M. Rump  complex realmax
% modified 09/23/15     S.M. Rump  avoid get(0,... in Octave, getformat
...
%AAAA Matlab V5.2 bug fix
  ...
  if ~isequal(format(end),'e')
    ...
    if c.complex
      cinf_ = nonzeros(cinf);
      cinf_ = max(abs(real(cinf_)),abs(imag(cinf_)));
      csup_ = nonzeros(csup);
      csup_ = max(abs(real(csup_)),abs(imag(csup_)));
