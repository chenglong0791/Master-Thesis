...
% modified 05/22/14     S.M. Rump  Octave 1i*inf
...
%
  ...
  if a.complex
    ...
    [m,n] = size(a.inf);
    if numel(a.inf)<2^31       % not huge
      if issparse(a.inf)
        if INTLAB_CONST.OCTAVE
        % Octave bug: 1i*inf = NaN+Infi
          x = nonzeros(a.inf(:)+complex(0,a.sup(:)));   
        else
          x = nonzeros(a.inf(:)+1i*a.sup(:));
        end
      else
