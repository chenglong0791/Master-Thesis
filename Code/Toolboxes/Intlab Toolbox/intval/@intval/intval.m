...
% modified 12/09/15     S.M. Rump  numel and ignore Matlab6.5 bug
% modified 12/10/15     S.M. Rump  dummy intervals for fast typecast
%                                    (thanks to Dr. Florian B�nger)
% modified 12/12/15     S.M. Rump  prod(size) to numel(s)
%

  global INTLAB_CONST

  if nargin==3                  % internal type cast
  ...
  elseif nargin==1
    ...
    % can never happen because there is @affari/intval
%     elseif isa(x,'affari')      % 1 parameter, affari
%       c = x;                    % affari are always of type intval
%       
