...
% modified 08/09/13     S.M. Rump  take care of subnormal numbers
...
%
  ...
  setround(0)
  if ( crnd~=0) & ( crnd<realmin )
    expoffset = 1;
    expoff = floor(log10(realmin/crnd));
    crnd = crnd*10^expoff;  % error-free
    absc = absc*10^expoff;
    if expon==0
      commonexp = commonexp + expoff;
    end
  else
    expoffset = 0;
  end
...
%AAAA  end bug fix
  ...
  while 1
    ...
    setround(round);
    if ( round*(absc-xrnd) <= 0 )
      if expon     % e-format
        if expoffset       % subnormal input
          e = 100*(str(end-2)-'0') + 10*(str(end-1)-'0') + str(end)-'0';
          stre = sprintf('%3d',e+expoff);
        else
          stre = str(end-2:end);
        end
        str = [ blanks(len-prec-7) str(1:digits+2) str(end-4:end-3) stre ];
        if signc == -1
          str(len-prec-7) = '-';
        end
      else                 % f-format
