...
%  f:R->R or f:R^n->R^n is a function to be called by f(x), possibly with
%  extra parameters, f(x,P1,P2,...). Optional input "see" is to see intermediate
%  results.
%
%The univariate case f:R->R, xs is an approximation to a K-fold root of f(x)=0.
...
% modified 05/01/14     S.M. Rump  typo
...
%
  ...
  if n==1                         % one nonlinear function
    ...
    [XX,xs] = verifynlssderiv(funfcn,xs,abs(K)-1,see,varargin{:});
    if see
...
%             X = intval(NaN);        % leading coefficient contains zero
    ...
    if ready && isempty(find(isnan(Y) | isinf(Y) ))
      ...
      X = intval(NaN(n+1,1));       % inclusion failed
    end
