...
%  starting at str(first) and ending at str(last)
  ...
  if any(cindex)       % complex input
    ...
    xmidcmplx(sum(cindex),1) = 1i;  % sqrt(-1)
    ...
    if any(iImagonly)
      ...
      setround(0)
      xmidcmplx(iImagonly) = xmidcmplx(iImagonly) * 1i;   % make complex
    ...
    if any(iRealimag)
      ...
      xinf = dec2dble(signcindex(iRealimag),mantre,expre,-1) + ...
             dec2dble(signim,mantim,expim,-1)*1i;
