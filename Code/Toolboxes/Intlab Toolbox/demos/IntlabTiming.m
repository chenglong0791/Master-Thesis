...
% modified 05/08/17     S.M. Rump  verifylocalmin, thanks to Kai T. Ohlhus
%

  TT = 10;                % Minimum time spend per test case to improve accuracy of timing
  rr = midrad(1,1e-8);    % radius for interval data
  disp(' ')
  disp(['Panasonic Intel i7 3540M 3.0 GHz, 64 bit Matlab ' version])
  ...
  if OptNonlinSys
    ...
    for n=N
      ...
      tic
      X = verifylocalmin(@test_h,approx,'SparseSPD',0,2);
      T1 = toc;
  ...
  if TimingDenseMatmul
    ...
    disp(' ')
    disp('Dense matrix multiplication, FastIVmult and SharpIVmult, time [sec]')
    disp(' ')
  ...
  if TimingSparseMatmul
    ...
    disp(' ')
    disp('Sparse matrix multiplication, FastIVmult and SharpIVmult, time [sec]')
    disp(' ')
    disp('  dimension   t1       t2   ')
    disp('------------------------------')
